...
#[clap(
    ...
    after_help = "EXAMPLES:\n\
    \n  Generate a 2048-bit RSA key pair and print to stdout:\n    cargo run --bin gen-key --stdout\n\
    \n  Generate a 4096-bit RSA key pair, print to stdout, and save to ./mykeys/:\n    cargo run --bin gen-key --alg rsa --key-size 4096 --out-dir ./mykeys --stdout\n\
    \n  Generate a 512-bit HMAC key and print as base64 to stdout:\n    cargo run --bin gen-key --alg hmac --stdout\n\
    \n  Save PEM-formatted RSA keys to ./certs/ only:\n    cargo run --bin gen-key --alg rsa --out-dir ./certs\n\
    \n  Print DER-formatted RSA keys as base64 to stdout and save to files:\n    cargo run --bin gen-key --alg rsa --stdout --out-dir ./certs --format der\n\
    \n  Generate PEM with LF line endings:\n    cargo run --bin gen-key --alg rsa --stdout --line-ending lf\n\
    \nNOTES:\n\
...
struct Cli {
    ...
    /// Output format: pem or der (RSA only, ignored for HMAC)
    #[clap(
        short,
        long,
        default_value = "pem",
        help = "Output format: pem or der (RSA only, ignored for HMAC)"
    )]
    format: OutputFormat,
...
fn generate_rsa(cli: &Cli, do_stdout: bool, do_file: bool) -> Result<()> {
    ...
    let priv_der = priv_key.to_pkcs1_der()?;
    ...
    if do_stdout {
        match cli.format {
            OutputFormat::Pem => {
                let priv_pem = priv_der.to_pem("RSA PRIVATE KEY", line_ending)?;
                let pub_pem = pub_der.to_pem("RSA PUBLIC KEY", line_ending)?;
    ...
    if do_file {
        ...
        match cli.format {
            OutputFormat::Pem => {
                let priv_path = out_dir.join("priv_key.pem");
                let pub_path = out_dir.join("pub_key.pem");

                if !cli.replace && (priv_path.exists() || pub_path.exists()) {
                    return Err(Error::RsaFileExists);
                }

                priv_der.write_pem_file(&priv_path, "RSA PRIVATE KEY", line_ending)?;
                pub_der.write_pem_file(&pub_path, "RSA PUBLIC KEY", line_ending)?;
                writeln!(stdout, "RSA private key saved to {}", priv_path.display())?;
                writeln!(stdout, "RSA public key saved to {}", pub_path.display())?;
            }
            OutputFormat::Der => {
                let priv_path = out_dir.join("priv_key.der");
                let pub_path = out_dir.join("pub_key.der");
...
fn generate_hmac(cli: &Cli, do_stdout: bool, do_file: bool) -> Result<()> {
    let key_bytes = cli.key_size.unwrap_or(HMAC_DEFAULT_KEY_SIZE) / 8;
    ...
    if do_file {
        ...
        let key_path = out_dir.join("hmac.key");
...
impl core::fmt::Display for Error {
    fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::result::Result<(), core::fmt::Error> {
        let msg = match self {
            ...
            Error::Pkcs1(error) => error.to_string(),
...
mod tests {
    ...
    fn test_rsa_key_generation_stdout() {
        ...
        cmd.assert()
            ...
            .stdout(predicate::str::contains("RSA PUBLIC KEY"));
    }
    ...
    #[test]
    fn test_rsa_key_generation_to_dir() {
        ...
        let priv_key = dir.path().join("priv_key.pem");
        let pub_key = dir.path().join("pub_key.pem");
        assert!(priv_key.exists(), "Private key file should exist");
    ...
    #[test]
    fn test_replace_flag_error_and_success_rsa() {
        let dir = tempdir().unwrap();
        let out_dir = dir.path();

        // Pre-create files to simulate existing keys
        let priv_key = out_dir.join("priv_key.pem");
        let pub_key = out_dir.join("pub_key.pem");
